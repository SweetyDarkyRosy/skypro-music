...
import { getSpecificSelectionList, getFavouriteTrackList } from '../api'
...
function TracklistCategory({ categoryId, categoryLabel }) {
  ...
  useEffect(() => {
    getSpecificSelectionList(categoryId).then((data) => {
        ...
        console.error(" - Error: Could not get a list of tracks from the category");
  ...
  useEffect(() => {
	...
	return (
      ...
      <TrackListSpecificH2>{ categoryLabel }</TrackListSpecificH2>
...
export default TracklistCategory;
